...
int read_students_grades(const char *filename, struct student_t ***students){
    if(filename == NULL || students == NULL) return -1;
    ...
    while(!feof(f))
    {
        ...
        /*
         char input[110];
         int res = fscanf(f, "%109[^\n]\n",input);
         char * name = strtok(input, "|");
         char * last_name = strtok(NULL, "|");
         char * course = strtok(NULL, "|");
         char *c_grade = strtok(NULL, "");

         if(name == NULL || last_name == NULL || course == NULL || c_grade == NULL)
         {
        ...
        if(student)
        {
            ...
            else
            {
                ...
                *((student->courses + course_id)->grades + (student->courses + course_id)->number_of_grades++) = grade;
                // printf("Grade assigned: %d\n", *((student->courses + course_id)->grades + (student->courses + course_id)->number_of_grades - 1));
            }
        }
        else
        {
...
int write_student_grades(const char *filename, struct student_t** students, const char* name, const char* last_name){
    if(filename == NULL || students == NULL || name == NULL || last_name == NULL) return -1;
...
int write_course_grades(const char *filename, struct student_t** students, const char* course){
    if(filename == NULL || students == NULL || course == NULL) return -1;
...
int add_grade(struct student_t*** students, const char* name, const char* last_name, const char* course, int grade){
    if(name == NULL || last_name == NULL || students == NULL || course == NULL) return -1;
...
int read_students_grades_bin(const char *filename, struct student_t ***students) {
    ...
    while (1) {
        if (fread(name, sizeof(char), 1, f) != 1) break;

        if (fread(name + 1, sizeof(char), (unsigned char)*name, f) != (unsigned char)*name)
        {
            free_students_grades(local_students);
            fclose(f);
            return -3;
        }
        *(name + (unsigned char)*name + 1) = '\0';

        if (fread(last_name, sizeof(char), 1, f) != 1 ||
            fread(last_name + 1, sizeof(char), (unsigned char)*last_name, f) != (unsigned char)*last_name) {
            free_students_grades(local_students);
            fclose(f);
            return -3;
        }
        *(last_name + (unsigned char)*last_name + 1) = '\0';

        if (fread(course, sizeof(char), 1, f) != 1 ||
            fread(course + 1, sizeof(char), (unsigned char)*course, f) != (unsigned char)*course) {
            free_students_grades(local_students);
            fclose(f);
            return -3;
        }
        *(course + (unsigned char)*course + 1) = '\0';

        if (fread(&grade, sizeof(char), 1, f) != 1)
        {
            free_students_grades(local_students);
            fclose(f);
            return -3;
