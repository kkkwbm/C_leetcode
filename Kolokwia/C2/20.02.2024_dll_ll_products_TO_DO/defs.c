...
struct products_t ** add_product(struct products_t ** products, char * product_name, int price, int (*wsk)(int)){
    ...
    if(product_name == NULL)
    {
        ...
        /*
        while (products_in_category > 0) {
            free((*(products + (--products_in_category)))->product_name);
            free(*(products + products_in_category));
        }
        free(products);
        return NULL;
         */
...
void free_list(struct double_linked_list_t* lists){
    ...
    while (node != NULL)
    {
        struct node_t *next = node->next; // Save next node before freeing the current one
        struct categories_t *category = node->categories;
        if (category != NULL)
        {
            free(category->name); // Free category name
            struct products_t **product_ptr = category->products;
            while (product_ptr != NULL && *product_ptr != NULL) // Traverse using pointers
            {
                free((*product_ptr)->product_name); // Free product name
                free(*product_ptr); // Free product struct
                product_ptr++; // Move to next product
            }
            free(category->products); // Free the products pointer array
            free(category); // Free the category struct
        }
        free(node); // Free the node
        node = next; // Move to next node
    }
    free(lists); // Free the list struct
}

void free_list2(struct double_linked_list_t* lists){
    if (lists == NULL) return;
    struct node_t *prev;
    struct node_t *node = lists->head;
    while (node != NULL)
